...
document.addEventListener(
    'DOMContentLoaded', function() {
      ...
      initNavigation();
        ...
        // if (activeModule) {
        //     loadDataForModule(activeModule.id);
        // }
    });

function initRoleAndPermissions() {
...
function initNavigation() {
  ...
  navItems.forEach(item => {
        ...
        if (module) {
            ...
            if (moduleRole && !moduleRole.split(',').includes(currentUser.role)) {
                item.style.display = 'none'; // 隐藏无权访问的导航项
            } else if (!firstAvailableModuleId) {
                firstAvailableModuleId = targetId; // 记录第一个可访问的模块ID
            }
        }
    });
    ...
    // Fallback to module1 if nothing found (shouldn't happen usually)
    if (!firstAvailableModuleId) {
        firstAvailableModuleId = 'module1';
    }

    if (firstAvailableModuleId) {
        const firstModule = document.getElementById(firstAvailableModuleId);
        const firstNavItem =
            document.querySelector(`[data-target="${firstAvailableModuleId}.html"]`);
        if (firstModule)
            firstModule.classList.add('active');
        if (firstNavItem)
            firstNavItem.classList.add('active');
        
        // Load data for the initial module
        loadDataForModule(firstAvailableModuleId);
    }
    ...
    navItems.forEach(item => {
        item.addEventListener(
            'click', function() {
                const targetId = this.getAttribute('data-target').replace('.html', '');
...
function loadDataForModule(moduleId) {
    switch (moduleId) {
    case 'module3':
        loadTaskModule();
        break;
...
function openPaperReviewModal(studentName, paperTitle, paperId, readOnly = false) {
    currentReviewPaperId = paperId;
    document.getElementById('review-student-name').textContent = studentName;
    document.getElementById('review-paper-title').textContent = paperTitle;
    ...
    .then(res => res.json())
    .then(paper => {
        contentArea.value = paper.reviewComment || '';
        scoreInput.value = paper.score || '';
    })
    .catch(err => {
        console.error(err);
        contentArea.value = '加载失败';
    });
}

function closePaperReviewModal() {
    document.getElementById('paper-review-modal').style.display = 'none';
    currentReviewPaperId = null;
}

async function submitPaperReview(event) {
    event.preventDefault();
    if (!currentReviewPaperId) return;

    const content = document.getElementById('review-content').value;
    const score = document.getElementById('review-score').value;

    try {
        const res = await fetch(`/api/paper/review/${currentReviewPaperId}`, {
            method: 'POST',
            headers: { 
    ...
    try {
        ...
        if (res.ok) {
            ...
            try {
                ...
                errorMessage = '服务器响应错误 (' + res.status + ')';
            }
...
async function loadPaperModule() {
    const role = currentUser.role;
    
    // Bind upload event for student
    if (role === 'student') {
        const fileInput = document.getElementById('paper-upload-input');
        if (fileInput) {
            fileInput.onchange = uploadPaper;
        }
    }

    try {
        const res = await fetch('/api/paper/list', {
            headers: { 'X-User-Id': currentUser.id || 1 }
        });
        ...
        if (role === 'student') {
            // Student view: show my paper
            const myPaper = papers.length > 0 ? papers[0] : null;
            
            const uploadArea = document.querySelector('#module7 .upload-area');
            ...
            // Teacher/Admin view: list all papers
            renderPaperReviewList(papers);

            // Bind search input
            const searchInput = document.querySelector('#module7 input[type="search"]');
            if (searchInput) {
                searchInput.oninput = (e) => {
                    const keyword = e.target.value.toLowerCase();
                    const filteredPapers = papers.filter(p => 
                        (p.studentName && p.studentName.toLowerCase().includes(keyword)) ||
                        (p.title && p.title.toLowerCase().includes(keyword))
                    );
                    renderPaperReviewList(filteredPapers);
                };
            }
        }
    } catch (error) {
    ...
    const reviewForm = document.getElementById('paper-review-form');
    if (reviewForm) {
        reviewForm.onsubmit = submitPaperReview;
    }
}

function renderPaperReviewList(papers) {
    const tbody = document.getElementById('paper-review-list');
    if (!tbody) return;
    tbody.innerHTML = '';
    ...
    papers.forEach(paper => {
        ...
        const reviewStatusBadge = paper.reviewStatus === '已评审'
            ? `<span class="status-badge status-completed">已评审</span>`
            : paper.reviewStatus === '待评审'
              ? `<span class="status-badge status-pending">待评审</span>`
              : `<span>--</span>`;

        const submissionStatusBadge = paper.status === '已提交' || paper.filePath // Assuming if filePath exists, it's submitted
        ...
        if (currentUser.role === 'teacher') {
            if (!paper.filePath) {
                actionButton = `<button class="button-link" disabled>评审</button>`;
            } else {
                const buttonText = paper.reviewStatus === '已评审' ? '查看评审' : '评审';
                // Teacher can always edit
                actionButton = `<button class="button-link" onclick="openPaperReviewModal('${paper.studentName}', '${paper.title}', ${paper.id}, false)">${buttonText}</button>`;
            }
        } else { // admin
            // Admin is read-only
            actionButton = `<button class="button-link" onclick="openPaperReviewModal('${paper.studentName}', '${paper.title}', ${paper.id}, true)">查看</button>`;
        }

        tr.innerHTML = `<td>${paper.studentName || '未知'}</td> <td>${paper.title}</td>
                        <td data-role="admin">${paper.teacherName || '--'}</td>
                        <td>${submissionStatusBadge}</td> <td>${reviewStatusBadge}</td>
                        <td>${paper.score || '--'}</td> <td>${actionButton}</td>`;
        tbody.appendChild(tr);
    });

    // 重新应用角色可见性，因为是动态创建的
    const roleElements = document.querySelectorAll('#paper-review-list [data-role]');
    roleElements.forEach(element => {
        const allowedRoles = element.getAttribute('data-role').split(',');
        ...
        if (currentUser.role === 'student') {
            ...
            if (infoCard) {
                infoCard.innerHTML = ` <div class = "info-item">
                    <span class = "label"> 学生姓名</ span><p class = "value"> 图图</ p></ div>
                    <div class = "info-item"><span class = "label"> 指导教师</ span>
                    <p class = "value"> David</ p></ div><div class = "info-item topic">
                    <span class = "label"> 选题课题</ span>
                    <p class = "value"> 基于深度学习的图像识别系统研究</ p></ div>
                `;
            }
        }
...
function renderRecordsTable(records) {
    ...
    paginatedRecords.forEach(record => {
        ...
        const contentDisplay =
            record.content ? `<span style = "color:blue"> ${escapeHtml(record.content)} < / span >`
                           : '无内容';
        const exportButton =
            record.content ? 
            `<button class = "btn btn-secondary" style =
                   "padding: 4px 12px; font-size: 12px; margin-left:5px;" onclick =
                       "viewRecordFile('${escapeHtml(record.content)}', ${record.id})">
                        导出 / 打印</ button>`
                           :
            `<button class = "btn btn-secondary" style =
                               "padding: 4px 12px; font-size: 12px; margin-left:5px;" disabled>
                                   导出 / 打印</ button>`;
...
function openEditModal(id, currentComment) {
    ...
    if (teacherSelect) {
        teacherSelect.value =
            currentComment === '通过' || currentComment === '不通过' ? currentComment : '';
    }
        ...
        if (currentTaskInfo&& currentTaskInfo.studentSubmitted === 1 && currentTaskInfo.studentDraftPath) {
            ...
            if (previewArea) {
                ...
                previewArea.innerHTML =
                    '<div style="padding: 20px; text-align: center; color: #86868b;">学生尚未提交初稿</div>';
            }
        }
    ...
    if (currentUser.role === 'teacher') {
        ...
        if (submitBtn) {
            ...
            if (currentTaskInfo&& currentTaskInfo.adminStatus === 'approved') {
                ...
                // 被退回，允许重新提交
                submitBtn.disabled = !currentTaskInfo || !currentTaskInfo.teacherRevisionPath ||
                                     (currentTaskInfo.studentSubmitted !== 1);
        ...
        if (currentTaskInfo&& currentTaskInfo.teacherSubmitted === 1 && currentTaskInfo.teacherRevisionPath) {
            ...
            if (previewArea) {
                ...
                previewArea.style.borderRadius = '8px';
                previewArea.innerHTML =
                    '<p style="margin: 0; color: #86868b;">教师尚未提交修改稿</p>';
            }
        }
    }
}
...
function renderTaskReviewList(tasks) {
    ...
    tasks.forEach(task => {
        ...
        if (task.adminStatus === 'approved') {
            statusHtml =
                '<span style="color: #34C759; font-weight: 500;">已通过</span> ' +
                '<button class="btn btn-secondary" style="color: var(--color-danger-text); margin-left: 10px;" onclick="deleteTaskRecord(' +
                task.id + ')">删除</button>';
        } else if (task.adminStatus === 'returned' && task.teacherSubmitted === 1) {
            // 退回后重新提交的，显示为待审核状态
            statusHtml =
                '<span style="color: #86868B; font-weight: 500;">待审核（已退回后重新提交）</span> ' +
                '<button class="btn btn-primary" onclick="approveTask(' + task.id +
                ')">通过</button> ' +
                '<button class="btn btn-secondary" style="color: var(--color-danger-text);" onclick="returnTask(' +
                task.id + ')">退回</button>';
